...
export const EventCreatedModal = ({ companyName, onAddTopic, onSkip }: { companyName: string; onAddTopic: () => void; onSkip: () => void }) => (
  <div className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center p-4" onClick={onSkip}>
    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl p-5" onClick={e => e.stopPropagation()}>
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Event created</h2>
          <p className="text-sm text-gray-500 dark:text-slate-400">
            Want to add a chat topic to announce this to {companyName}'s chat?
          </p>
        </div>
      </div>
