...
const ProfileSheet = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[60] bg-slate-900 rounded-t-2xl shadow-2xl border-t border-slate-800 animate-in slide-in-from-bottom duration-200">
        <div className="max-w-2xl mx-auto px-4 py-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="font-semibold text-slate-100">{currentUser?.username}</div>
              <div className="flex items-center gap-1 mt-0.5">
                <Coins className="w-3.5 h-3.5 text-blue-400" />
                <span className="text-sm font-medium text-blue-300">{currentUser?.coins.toLocaleString()}</span>
          ...
          <div className="space-y-1 mb-4">
            {!isAnon && (
              <>
                <button onClick={() => go('/profile')} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-800 transition-colors text-left">
                  <User className="w-5 h-5 text-slate-400" />
                  <span className="text-sm font-medium text-slate-100">My Profile</span>
                </button>
                <button onClick={() => go('/settings')} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-800 transition-colors text-left">
            ...
            {currentUser?.isAdmin && (
              <>
                <button onClick={() => go('/feedback-admin')} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-800 transition-colors text-left">
                  <MessageSquare className="w-5 h-5 text-blue-400" />
                  <span className="text-sm font-medium text-slate-100">Feedback</span>
                </button>
                <button onClick={() => go('/admin')} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-800 transition-colors text-left">
                  <Shield className="w-5 h-5 text-blue-400" />
                  <span className="text-sm font-medium text-slate-100">Admin Panel</span>
                </button>
              </>
            )}
            {isAnon && (
              <button onClick={handleDeleteSession} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-rose-900/20 transition-colors text-left">
                <Trash2 className="w-5 h-5 text-rose-400" />
                <span className="text-sm font-medium text-rose-400">Delete session</span>
              </button>
            )}
          </div>
          ...
          {isAnon ? (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors text-sm font-medium text-slate-300"
            >
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[60] bg-slate-900 rounded-t-2xl shadow-2xl border-t border-slate-800 animate-in slide-in-from-bottom duration-200">
        <div className="max-w-2xl mx-auto px-4 py-5">
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm font-semibold text-slate-100">Guest</div>
            <button onClick={onClose} className="p-1.5 rounded-lg text-slate-500 hover:text-slate-300 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="max-w-md sm:max-w-5xl mx-auto w-full px-4 sm:px-8 flex items-center justify-between h-14">
          <Link to="/" className="flex items-center gap-2 flex-shrink-0">
            <div className="flex items-baseline gap-0.5">
              <span className="font-semibold text-slate-300 text-sm tracking-tight">Layoff</span>
              <span className="font-black text-blue-400 text-sm tracking-tight">Live</span>
          ...
          <nav className="flex items-center gap-1">
            ...
            <button
              ...
              className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-md font-medium text-xs sm:text-sm border-2 transition-colors ${
            ...
            {isLoggedIn ? (
              <button
                ...
                className="flex items-center gap-0.5 sm:gap-1 ml-0.5 sm:ml-1 bg-slate-800 hover:bg-slate-700 rounded-full px-2 sm:px-3 py-1 sm:py-1.5 transition-colors"
              >
                <User className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-slate-200" />
                <ChevronDown className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-slate-500" />
              </button>
            ) : isAnon ? (
              <button
                onClick={() => setShowProfile(true)}
