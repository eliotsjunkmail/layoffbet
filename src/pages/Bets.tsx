  ...
  return (
    <Layout>
      <div className="flex items-center gap-2 mb-4">
        <button onClick={() => navigate('/')} className="flex items-center text-slate-400 hover:text-slate-200 transition-colors flex-shrink-0 p-1">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-bold text-slate-100">My Bets</h1>
      </div>
      ...
      {(currentUser && userStats) || !currentUser ? (
        <div className="mb-5 -mx-4 px-4 pb-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl p-2.5 sm:p-4 text-center shadow-sm flex flex-col">
              <div className="text-xs text-slate-400 uppercase font-medium mb-1 sm:mb-2">Coins</div>
              <div className="text-2xl sm:text-3xl font-bold text-blue-400 flex-1 flex items-center justify-center">{currentUser && userStats ? userStats.coins : Math.max(0, anonCoins - anonCoinsSpent)}</div>
              <div className="text-xs text-slate-500 mt-0.5 sm:mt-1">remaining</div>
            </div>
            {currentUser && userStats && (
              <>
                <div className="bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl p-2.5 sm:p-4 text-center shadow-sm flex flex-col">
                  <div className="text-xs text-slate-400 uppercase font-medium mb-1 sm:mb-2">My Bets</div>
                  <div className="text-2xl sm:text-3xl font-bold text-blue-400 flex-1 flex items-center justify-center">{userStats.totalBets}</div>
                  <div className="text-xs text-slate-500 mt-0.5 sm:mt-1">{userStats.activeBets} active</div>
                </div>
                <div className="bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl p-2.5 sm:p-4 text-center shadow-sm flex flex-col">
                  <div className="text-xs text-slate-400 uppercase font-medium mb-1 sm:mb-2">Wagered</div>
                  <div className="text-2xl sm:text-3xl font-bold text-blue-400 flex-1 flex items-center justify-center">{userStats.totalBetAmount}</div>
                  <div className="text-xs text-slate-500 mt-0.5 sm:mt-1">coins</div>
                </div>
              </>
            )}
            {!currentUser && (
              <>
                <div className="bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl p-2.5 sm:p-4 text-center shadow-sm flex flex-col">
                  <div className="text-xs text-slate-400 uppercase font-medium mb-1 sm:mb-2">My Bets</div>
                  <div className="text-2xl sm:text-3xl font-bold text-blue-400 flex-1 flex items-center justify-center">{Object.keys(anonVotedEvents).length}</div>
                  <div className="text-xs text-slate-500 mt-0.5 sm:mt-1">{Object.keys(anonVotedEvents).length} active</div>
                </div>
                <div className="bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl p-2.5 sm:p-4 text-center shadow-sm flex flex-col">
                  <div className="text-xs text-slate-400 uppercase font-medium mb-1 sm:mb-2">Wagered</div>
                  <div className="text-2xl sm:text-3xl font-bold text-blue-400 flex-1 flex items-center justify-center">{anonCoinsSpent}</div>
                  <div className="text-xs text-slate-500 mt-0.5 sm:mt-1">coins</div>
                </div>
              </>
            )}
      ...
      <div className="flex bg-slate-800/60 rounded-xl p-1 mb-5 gap-1">
        {([
          ...
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium rounded-lg transition-all ${tab === t.id ? 'bg-slate-700 text-slate-100 shadow' : 'text-slate-400 hover:text-slate-200'}`}
          >
            {t.label}
            {t.count > 0 && (
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${tab === t.id ? 'bg-violet-900/50 text-violet-400' : 'bg-slate-700 text-slate-400'}`}>{t.count}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {totalShown === 0 ? (
        <div className="text-center py-16">
          ...
          {tab === 'active' && (
            <button onClick={() => navigate('/')} className="text-violet-400 text-sm hover:underline">Browse predictions →</button>
          )}
        </div>
      ) : (
        <div className="space-y-6">
          {shown.map(({ companyName, slug, items }) => (
            <section key={slug}>
              ...
              >
                <span className="text-sm font-semibold text-slate-300 group-hover:text-violet-400 transition-colors">{companyName}</span>
                <ChevronRight className="w-3.5 h-3.5 text-slate-600 group-hover:text-violet-500 transition-colors" />
              </Link>
                    ...
                    return (
                      <div key={event.id}>
                        <div
                          ...
                          className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3.5 hover:border-violet-700 hover:shadow-sm transition-all cursor-pointer"
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-xs font-medium">
                              {won && <span className="text-emerald-400 flex items-center gap-0.5"><CheckCircle className="w-3 h-3" /> Won</span>}
                              {lost && <span className="text-rose-400">Lost</span>}
                              {status === 'expired' && <span className="text-amber-400">Expired</span>}
                            </span>
                          </div>
                          <p className="text-sm font-medium text-slate-100 leading-snug line-clamp-2 mb-2">{event.title}</p>
                          ...
                          <div className="flex justify-between text-xs">
                            {dominant === 'yes' ? <span className="text-emerald-400 font-semibold">YES {pct}%</span> : <span className="text-slate-700">·</span>}
                            <span className="text-slate-500">{event.yesPool + event.noPool} coins</span>
                            {dominant === 'no' ? <span className="text-rose-400 font-semibold">NO {pct}%</span> : <span className="text-slate-700">·</span>}
                          </div>
                          {status === 'resolved' && event.outcome && bet && (
                            <div className={`mt-2.5 text-xs text-center font-medium py-1.5 rounded-lg ${won ? 'bg-emerald-900/20 text-emerald-400' : 'bg-rose-900/20 text-rose-400'}`}>
                  ...
                  return (
                    <div key={event.id}>
                      <SwipeCard
                        ...
                        demoActive={false}
                        onClick={() => navigate(`/event/${event.id}`)}
                        cardClassName={`bg-slate-800 border rounded-xl px-4 py-3.5 shadow-sm [@media(hover:hover)]:hover:shadow-md select-none transition-shadow
                          ${flash && swipeFlash?.side === 'yes' ? 'border-emerald-500 bg-emerald-900/20' :
                            flash && swipeFlash?.side === 'no' ? 'border-rose-500 bg-rose-900/20' :
                            'border-violet-800'}`}
                      >
                        ...
                        <div className="flex justify-between text-xs">
                          {dominant === 'yes' ? <span className="text-emerald-400 font-semibold">YES {pct}%</span> : <span className="text-slate-700">·</span>}
                          <span className="text-slate-500 flex items-center gap-0.5"><Clock className="w-3 h-3" />{timeUntil(event.expiresAt)}</span>
                          {dominant === 'no' ? <span className="text-rose-400 font-semibold">NO {pct}%</span> : <span className="text-slate-700">·</span>}
                        </div>
                      </SwipeCard>
                    </div>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-100 text-slate-900 px-5 py-2.5 rounded-full text-sm font-medium shadow-lg z-50 pointer-events-none">
          {toast}
        </div>
      )}
    </Layout>
